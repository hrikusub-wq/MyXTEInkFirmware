...
約6.5MB)を使うことにし、SDカードの`/fonts/NotoSansJp_12.cpfont`に置く前提で
...
- **CLOCK IN STATUS BAR**: ON時、ホーム画面のステータスバー左側(元は
  ...
  ないため)。
- **SYSTEM FONT**: LEFT/RIGHTで`MiniFontImpl`とSDカードの`/fonts/*.cpfont`を
...
**既知の制限**: `/fonts`ディレクトリのスキャンは設定画面を開くたびではなく
